...
class AskUserEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        '''
        # ignore any modifier key presses.
        if event.sym in {
                tcod.event.K_LSHIFT, 
                tcod.event.K_RSHIFT, 
                tcod.event.K_LCTRL, 
    ...
    def ev_mousebuttondown(self, event: tcod.event.MouseButtonDown) -> Optional[ActionOrHandler]:
        '''
        By default a mouse click exits this input handler
        '''
        return self.on_exit()

    def on_exit(self) -> Optional[ActionOrHandler]:
        '''
        Called when the user is trying to exit or cancelling an action.
...
class AreaRangedAttackHandler(SelectIndexHandler):
    '''
    Handles targetting an area within a radius. Any entity within the area will be affected.
    '''
    def __init__(
            self, 
            engine: Engine, 
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        # Draw a rectangle around the targetted area, so the player can see the affected tiles.
        console.draw_frame(
            x=x - self.radius - 1, 
            y=y - self.radius -1, 
            width=self.radius ** 2, 
